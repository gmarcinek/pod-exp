...
export function DebateTranscript({ entries }: DebateTranscriptProps) {
  const agent1 =
    entries.find(
      (candidate): candidate is DebateTurnEntry =>
        candidate.type === "turn" && candidate.slot === "s1",
    )?.agent ?? "Agent 1";
  const agent2 =
    entries.find(
      (candidate): candidate is DebateTurnEntry =>
        candidate.type === "turn" && candidate.slot === "s2",
    )?.agent ?? "Agent 2";
    ...
    return (
      <div className={styles.empty}>
        ...
        <div className={styles.emptySubtitle}>
          Wybierz agenta i zadaj pytanie
        </div>
      </div>
    );
  }
      ...
      {entries.map((entry) => {
        ...
        if (entry.type === "error") {
          return (
            <div
              key={entry.id}
              className={`${styles.chatError} ${styles.entryFullWidth}`}
            >
        ...
        if (entry.type === "analysis") {
          return (
            <div
              key={entry.id}
              className={`${styles.debateEntry} ${styles.analyzer}`}
            >
              <div className={styles.entryHeader}>
                <span
                  className={`${styles.agentTag} ${styles.agentTagAnalyzer}`}
                >
                  {entry.title}
                </span>
              </div>
        ...
        return (
          <div
            key={entry.id}
            className={`${styles.debateEntry} ${entry.slot === "s1" ? styles.slotOne : styles.slotTwo}`}
          >
            <div className={styles.entryHeader}>
              <span
                className={`${styles.agentTag} ${entry.slot === "s1" ? styles.agentTagOne : styles.agentTagTwo}`}
              >
                {entry.agent}
              </span>
